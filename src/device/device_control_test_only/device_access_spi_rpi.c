...
/** The control_write_command() in this file is modified to enable the user 
 * to write a stream of bytes, which is supposedly a write command, into the device,
 * without trying to interpret as the resid, cmd_id, payload_len format.
 * This function has the same API as the fwk_rtos control_write_command so that the higher layers
 * don't need to change. When testing low level functionality, resid and cmd are sent as 0 by the caller.
 * This condition is interpreted in this function as the caller wanting to exercise the low level functionality,
 * and payload_len number of bytes from the payload[] array are sent to the device.
 * 
 */
control_ret_t
...
{
  // resid and cmd fields are not used but kept here to keep the API compatible with the fwk_rtos control_write_command so the
  // caller function doesn't need to change
  ...
  do
  {
      ...
      bcm2835_spi_transfern((char *)data_sent_recieved, 1);
  }while(data_sent_recieved[0] == CONTROL_COMMAND_IGNORED_IN_DEVICE);
...
/** The control_read_command() in this file is modified to enable the user 
 * to write a stream of bytes, which is supposedly a read command, into the device,
 * without trying to interpret as the resid, cmd_id, payload_len format.
 * This function has the same API as the fwk_rtos control_read_command so that the higher layers
 * don't need to change. When testing low level functionality, resid and cmd are sent as 0 by the caller.
 * This condition is interpreted in this function as the caller wanting to exercise the low level functionality,
 * and payload_len number of bytes from the payload[] array are sent to the device.
 * 
