...
opt_t options[] = {
            ...
            {"--set-aec-filter",       "-set-filter", "Get AEC filter", 0}, // TODO pass filename as an option
...
void get_one_filter(int32_t mic_index, int32_t far_index, std::string filename, uint32_t buffer_length, cmd_t * commands, size_t num_commands, bool flag_buffer_get)
{
    Command command;
    control_ret_t ret;
    printf("filename = %s\n", filename.c_str());
...
control_ret_t special_cmd_aec_filter(const char *filename, bool flag_buffer_get, cmd_t * commands, size_t num_commands)
{
    Command command;
    ...
    for(int far_index=0; far_index<num_farends.i32; far_index++)
    {
        for(int mic_index=0; mic_index<num_mics.i32; mic_index++)
        {
            ...
            filter_name = filter_name + ".f" + std::to_string(far_index) + ".m" + std::to_string(mic_index) ;
