...
def get_dummy_files():
    ...
    build_dir = (
        local_build_folder
        if local_build_folder.is_dir()
        else Path(__file__).parents[1] / "release"
    )
    ...
    assert (
        host_bin_path.is_file() or host_bin_copy.is_file()
    ), f"host app binary not found here: {host_bin}"
    if (not host_bin_copy.is_file()) or (
        host_bin_path.is_file() and host_bin_copy.is_file()
    ):
        shutil.copy2(host_bin_path, host_bin_copy)

    assert (
        cmd_map_dummy_path.is_file() or cmd_map_path.is_file()
    ), f"not found {cmd_map_dummy_path}"
    if (not cmd_map_path.is_file()) or (
        cmd_map_dummy_path.is_file() and cmd_map_path.is_file()
    ):
    ...
    assert (
        device_dummy_path.is_file() or device_path.is_file()
    ), f"not found {device_dummy_path}"
    if (not device_path.is_file()) or (
        device_dummy_path.is_file() and device_path.is_file()
    ):
    ...
    if platform.machine() == "armv7l":
        ...
        if (not dfu_app_bin_copy.is_file()) or (
            dfu_app_bin_path.is_file() and dfu_app_bin_copy.is_file()
        ):
            shutil.copy2(dfu_app_bin_path, dfu_app_bin_copy)
        assert (
            dfu_yaml_file1_path.is_file() or dfu_yaml_file1_copy.is_file()
        ), f"DFU YAML file not found here: {dfu_yaml_file1}"
        if (not dfu_yaml_file1_copy.is_file()) or (
            dfu_yaml_file1_path.is_file() and dfu_yaml_file1_copy.is_file()
        ):
            shutil.copy2(dfu_yaml_file1_path, dfu_yaml_file1_copy)
        assert (
            dfu_yaml_file2_path.is_file() or dfu_yaml_file2_copy.is_file()
        ), f"DFU YAML file not found here: {dfu_yaml_file2}"
        if (not dfu_yaml_file2_copy.is_file()) or (
            dfu_yaml_file2_path.is_file() and dfu_yaml_file2_copy.is_file()
        ):
...
def execute_command(
    ...
    if cmd_name[0] != "-":
        ...
        if len(words) == 2:  # cmd and 1 value
            return words[
                1
            ]  # To avoid changing all the other tests that call execute_command and expect a single value and not an array
        else:
            return words[1:]
    else:
        return words
