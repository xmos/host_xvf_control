...
def test_execute_cmd_list():
    test_dir, host_bin, control_protocol, cmd_map_name, _ = test_utils.get_dummy_files()
    print("\n")
...
def test_version():
    test_dir, host_bin, control_protocol, _, _ = test_utils.get_dummy_files()
    print("\n")
...
if platform.machine() == "armv7l":
    def test_dfu_app():
        ...
        cmd_output = test_utils.execute_command(dfu_app_bin, control_protocol, test_dir, "-av")
        assert str(cmd_output[0]) == changelog_version

        # test "xvf_dfu -u i2c -av -vvv"
        cmd_output = test_utils.execute_command(dfu_app_bin, control_protocol, test_dir, "-av -vvv")
        ...
        # test "xvf_dfu -u i2c -av -vvv"
        cmd_output = test_utils.execute_command(dfu_app_bin, control_protocol, test_dir, "-vvv -av")
        cmd_output_lines = " ".join(cmd_output).split("\n")
        assert cmd_output_lines[0] == "Verbose mode enabled"
        assert cmd_output_lines[2] == changelog_version

        # test "xvf_dfu -u i2c -h"
        cmd_output = test_utils.execute_command(dfu_app_bin, control_protocol, test_dir, "-h")
        cmd_output_lines = " ".join(cmd_output).split("\n")
        ...
        cmd_output = test_utils.execute_command(dfu_app_bin, control_protocol, test_dir, "--upload-start --version", expect_success=False)

        # test "xvf_dfu -u i2c --version --upload-start " returns an error, as --upload-start values is missing
        cmd_output = test_utils.execute_command(dfu_app_bin, control_protocol, test_dir, "--upload-start  -us", expect_success=False)

        # test "xvf_dfu -u i2c --version --upload-start 999999" returns an error, as --upload-start values is out of range
        cmd_output = test_utils.execute_command(dfu_app_bin, control_protocol, test_dir, "--upload-start 999999 --version", expect_success=False)

        # test "xvf_dfu -u i2c --reboot --upload-start  100" returns an error, as --upload-start is not valid with --reboot
        cmd_output = test_utils.execute_command(dfu_app_bin, control_protocol, test_dir, "--reboot -us 100", expect_success=False)
